...
alias history='noglob history -i'
...
alias ll='ls -lh --time-style=long-iso'        # Lists human readable sizes.
alias lr='ll -R --time-style=long-iso'         # Lists human readable sizes, recursively.
alias la='ll -A --time-style=long-iso'         # Lists human readable sizes, hidden files.
alias lm='la | "$PAGER" --time-style=long-iso' # Lists human readable sizes, hidden files through pager.
alias lk='ll -Sr --time-style=long-iso'        # Lists sorted by size, largest last.
alias lt='ll -tr --time-style=long-iso'        # Lists sorted by date, most recent last.
alias lc='lt -c --time-style=long-iso'         # Lists sorted by date, most recent last, shows change time.
alias lu='lt -u --time-style=long-iso'         # Lists sorted by date, most recent last, shows access time.

if [[ ${(@M)${(f)"$(ls --version 2>&1)"}:#*GNU *} ]]; then
  alias lx='ll -XB --time-style=long-iso'        # Lists sorted by extension (GNU only).
fi

# Grep
if zstyle -t ':prezto:module:utility:grep' color; then
  export GREP_COLOR=${GREP_COLOR:-'30;43'}            # BSD.

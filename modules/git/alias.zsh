...
zstyle -s ':prezto:module:git:log:medium' format '_git_log_medium_format' \
  || _git_log_medium_format='%C(dim white)Commit:%C(reset) %C(yellow)%H%C(auto)%d%n%C(dim white)Author:%C(reset) %C(cyan)%an <%ae>%n%C(dim white)Date:%C(reset)   %C(blue)%ai (%ar)%C(reset)%n%+B'
...
if ! zstyle -t ':prezto:module:git:alias' skip; then
  ...
  alias gfc='git clone'
  alias gfC='git-clone-into-owner'
  ...
  if type 'git flow' &> /dev/null; then
    # Flow (F)
  ...
  alias gwD='git diff --no-ext-diff --word-diff=color'
